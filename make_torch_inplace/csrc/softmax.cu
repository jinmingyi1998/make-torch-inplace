...
template <typename T>
__global__ void attn_softmax_inplace_(T *input, long long rows, int cols) {
...
void attn_softmax_inplace_forward_(at::Tensor input, long long rows, int cols) {
  ...
  if (input.dtype() == torch::kFloat32) {
    attn_softmax_inplace_<float>
        <<<grid, block>>>((float *)input.data_ptr(), rows, cols);
  } else if (input.dtype() == torch::kFloat16) {
    attn_softmax_inplace_<at::Half>
        <<<grid, block>>>((at::Half *)input.data_ptr(), rows, cols);
  } else {
    attn_softmax_inplace_<at::BFloat16>
        <<<grid, block>>>((at::BFloat16 *)input.data_ptr(), rows, cols);
  }
}

template <typename T>
__global__ void attn_softmax_inplace_grad_(T *output, T *d_ov, T *values,
                                           long long rows, int cols_output,
                                           int cols_values) {
  int threadidx_x = threadIdx.x / 32;
  int threadidx_y = threadIdx.x % 32;
  long long row_offset = (long long)(blockIdx.x * 4 + threadidx_x);
...
void attn_softmax_inplace_backward_(at::Tensor output, at::Tensor d_ov,
  ...
  if (output.dtype() == torch::kFloat32) {
    attn_softmax_inplace_grad_<float><<<grid, block>>>(
        (float *)output.data_ptr(), (float *)d_ov.data_ptr(),
        (float *)values.data_ptr(), rows, cols_output, cols_values);
  } else if (output.dtype() == torch::kFloat16) {
    attn_softmax_inplace_grad_<at::Half><<<grid, block>>>(
        (at::Half *)output.data_ptr(), (at::Half *)d_ov.data_ptr(),
        (at::Half *)values.data_ptr(), rows, cols_output, cols_values);
  } else {
    attn_softmax_inplace_grad_<at::BFloat16><<<grid, block>>>(
        (at::BFloat16 *)output.data_ptr(), (at::BFloat16 *)d_ov.data_ptr(),
        (at::BFloat16 *)values.data_ptr(), rows, cols_output, cols_values);
  }
}
